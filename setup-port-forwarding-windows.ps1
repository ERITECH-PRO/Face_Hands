# PowerShell Port Forwarding Setup for ESP32 Camera
...
function Test-PortForwarding {
    ...
    Write-Host "🌐 Testing external access to $PUBLIC_IP`:$EXTERNAL_PORT..." -ForegroundColor Yellow
    try {
        $tcpClient = New-Object System.Net.Sockets.TcpClient
        $tcpClient.Connect($PUBLIC_IP, $EXTERNAL_PORT)
        $tcpClient.Close()
        Write-Host "✅ Port forwarding is working!" -ForegroundColor Green
        return $true
    } catch {
        Write-Host "❌ Port forwarding not working" -ForegroundColor Red
        Write-Host "   Check:" -ForegroundColor Yellow
        Write-Host "   - Router port forwarding rules" -ForegroundColor White
        Write-Host "   - Firewall settings" -ForegroundColor White
        Write-Host "   - ISP blocking ports" -ForegroundColor White
        return $false
    }
}
...
function Update-Config {
    ...
    try {
        $dockerStatus = docker ps --filter "name=face-hands" --format "table {{.Names}}" | Select-Object -Skip 1
        ...
        Write-Host "⚠️  Docker not available or service not running" -ForegroundColor Yellow
    }
}
