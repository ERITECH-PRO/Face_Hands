# PowerShell VPN Setup for ESP32-VPS Connectivity (Fixed)
...
if ($Mode -eq "server") {
    Write-Host "Setting up VPN Server (Local Network)..." -ForegroundColor Blue
...
"@
    ...
    try {
        ...
        Write-Host "External IP: $publicIP`:$WG_PORT" -ForegroundColor Yellow
    } catch {
        Write-Host "External IP: YOUR_PUBLIC_IP`:$WG_PORT" -ForegroundColor Yellow
    }
...
"@
    ...
    if (Test-Path ".env") {
        ...
        $envContent = $envContent -replace 'STREAM_URL=.*', 'STREAM_URL=http://192.168.110.150:81/stream'
